#!/bin/sh
# runant.sh: simple shell script for automatic build/test/... using Ant.
# Takes one command line argument: the revision number to check out and test
...
#   the root of the web directory for the project.  New directories
...
cd $WORK_DIR
rm -rf trunk-$1
svn checkout -r $1 $REPO trunk-$1
cd trunk-$1
cp properties/$PROPS ./build.properties
