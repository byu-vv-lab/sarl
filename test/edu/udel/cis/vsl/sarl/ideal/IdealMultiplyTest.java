...
/**
 * The class IdealMultiplyTest tests methods found in the
 * edu.udel.cis.vsl.sarl.ideal.common package using multiplications among
...
public class IdealMultiplyTest {
	...
	public void setUp() throws Exception {
		...
		intZero = idealFactory.intConstant(0).commit();
		constOnePointFive = idealFactory.constant(ratOnePointFive).commit();
		constNegPointTwoFive = idealFactory.constant(ratNegPointTwoFive)
				.commit();
		intOne = idealFactory.intConstant(1).commit();
		intTwo = idealFactory.intConstant(2).commit();
		intTen = idealFactory.intConstant(10).commit();
		Xobj = objectFactory.stringObject("X").commit();
		...
		zero = idealFactory.constant(realZero).commit();
		one = idealFactory.constant(realOne).commit();
		fifteen = idealFactory.constant(realFifteen).commit();
		five = idealFactory.constant(realFive).commit();
		realThree = numberFactory.rational("3");
		three = idealFactory.constant(realThree).commit();
	}
	 ...
	 *            - Polynomial
	 * 
	 * @return the value of an expression consisting of multiplication of two
	 *         polynomials
	 */
	 ...
	 *            - Polynomial
	 * 
	 * @return the value of an expression consisting of addition of two
	 *         polynomials
	 */
	...
	/**
	 * Multiplies two Constants of type real and returns a Constant with the
	 * same type
	 * 
	 * @param type
	 *            Constant
	...
	public void constantMultiply() {
		Constant result = (Constant) idealFactory.multiply(constOnePointFive,
				constNegPointTwoFive);
		RationalNumber expected = numberFactory.rational("-.375");

		out.println("constantMultiply: " + constOnePointFive + " * "
				+ constNegPointTwoFive + " = " + result);
	 ...
	 *            Polynomial
	 * 
	 * @return a polynomial of type Polynomial which is the multiplication of
	...
	public void mulPolyToPoly() {
		...
		NumericExpression p4 = idealFactory.add(
				multiply(idealFactory.intConstant(3), multiply(poly1, poly1)),
				intOne);
		NumericExpression p5 = idealFactory.add(
				idealFactory.multiply(intTwo, x4), p4);
	...
	/**
	 * Multiplies a polynomial with a monomial by forming the factorization and
	 * by factoring out the common factors that are produced from the two
	 * factorizations.
	 * 
	 * @param p1
	 *            a Monomial
	 ...
	 *            Polynomial
	 * 
	 * @return a polynomial of type Polynomial which is the multiplication of a
	 *         polynomial and a monomial (passed as arguments).
	 */
	@Test
	public void mulPolyToMonomial() {
		Polynomial poly1 = (Polynomial) x;

		Polynomial p1 = add(multiply(poly1, poly1), intOne).commit(); // x^2+1
		Polynomial p2 = multiply(intTen, poly1).commit(); // 10x
		NumericExpression x2 = multiply(poly1, poly1).commit(); // x^2
		NumericExpression p3 = idealFactory
				.add(idealFactory
						.multiply(intTen, idealFactory.multiply(x2, x)),
						p2); // 10x^3+10x

		Polynomial b1 = multiply(p2, p1); // 10x^3+10x

		assertEquals(p3, b1);
	}

	/**
	 * Multiplies two monomials by forming the factorization and by factoring
	 * out the common factors that are produced from the two factorizations.
	 * 
	 * @param p1
	 *            a Monomial
	...
	/**
	 * Multiplies a monomial to a monic by forming the factorization and by
	 * factoring out the common factors that are produced from the two
	 * factorizations.
	 * 
	 * @param p1
	 *            a Monomial
	 ...
	 * 
	 * @return a polynomial of type Polynomial which is the multiplication of a
	 *         monomial to a monic (passed as arguments).
	 */
	@Test
	...
	/**
	 * Multiplies two monics by forming the factorization and by factoring out
	...
	public void mulMonicToMonic() {
		...
		Polynomial p2 = multiply(poly1, poly2);
		NumericExpression p3 = multiply(
				multiply(poly1, multiply(poly1, poly1)), multiply(poly2, poly2));
	 ...
	 * and by factoring out the common factors that are produced from the two
	 * factorizations.
	 * 
	 * @param p1
	 *            a Monomial
	...
	public void mulMonicToPrimitivePower() {
		...
		Polynomial x2 = multiply(poly1, poly1);
		NumericExpression p2 = multiply(
				multiply(poly1, multiply(poly1, poly1)), poly2);
	...
	public void mulPrimitivePowerToPrimitive() {
		...
		Polynomial x2 = multiply(poly1, poly1).commit();
	...
	/**
	 * Multiplies a Constant and a Primitive by forming the factorization and by
	 * factoring out the common factors that are produced from the two
	 * factorizations.
	 * 
	 * @param p1
	 ...
	 * 
	 * @return a polynomial of type Polynomial which is the multiplication of a
	 *         constant and a primitive (passed as arguments).
	 */
	@Test
	public void mulConstantToPrimitive() {
		Polynomial poly1 = (Polynomial) x;
		NumericExpression p1 = idealFactory.multiply(intTen, x).commit();
	...
	/**
	 * Multiplies two rational numbers. Also checks if the first or second
