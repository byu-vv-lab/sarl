...
/**
 * Converted (failed) benchmark to allow analysis and submission for TRAC ticket
 * on issues Intended to perform performance analysis on polynomial-term
 * expansion with "divides"
 * 
 * @author danfried
 * 
 */
public class DividesTest {
	...
	@Ignore
	@Test
	/** TODO The assumption "z==x/y (i.e. divide(x,y))" doesn't 
	 * imply that 'z==x|y (i.e. divides(x, y))'.
	 * Thus the assertion "assertEquals(z, idealSimplifier.apply(
	 * preUniv.divides(y, x)))" fails.
	 **/
	public void test1() {

		/*
		 * out.println("here:  " + preUniv.divides(div, x) + "  " +
		 * preUniv.divides(div, x).type()); // + "  true? " +
		 * preUniv.divides(div, x)==trueExpr);
		 */
		assumption = preUniv.equals(z, preUniv.divide(x, y));
		// assumption = preUniv.equals(preUniv.divide(x, y), z);
		// assumption = preUniv.equals(preUniv.divides(div, x),
		// preUniv.trueExpression()); //div == x * n
		...
		// out.println("true? : " +
		// idealSimplifier.simplifyExpression(divPoly));

		// out.println("assumption: " + assumption);
		// out.println("works???? " + idealSimplifier.apply(preUniv.divides(y,
		// x)));
		...
		for (int i = 0; i < size; i++) {
			...
			divPoly = (Polynomial) preUniv.multiply(divPoly,
					preUniv.add(x, int3));
		...
		/*
		 * for(NumericExpression i : divCol){ start =
		 * System.currentTimeMillis();
		 * 
		 * out.print(i + " :  "); idealSimplifier.apply((Polynomial)i);
		 * 
		 * time = System.currentTimeMillis() - start; out.println("  time: " +
		 * time); }
		 */
	...
	/**
	 * Test on IdealSimplifier with increasing terms of an integer-based
	 * polynomial when applying and evaluating "divides"
	 * 
	 * @see IdealSimplifier
	 */
	@Ignore
	@Test
	/**
	 * TODO This test fails because preUniv.divides(x, y) doesn't
