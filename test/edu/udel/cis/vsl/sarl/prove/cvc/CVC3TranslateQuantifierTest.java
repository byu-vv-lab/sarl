...
public class CVC3TranslateQuantifierTest {
	...
	public void testTranslateExists() {
		...
		BooleanExpression xLessN = (BooleanExpression) expressionFactory
				.expression(SymbolicOperator.LESS_THAN, boolType, xConstant,
						NConstant);
		// 0 < x < N
		BooleanExpression zeroLessXLessN = (BooleanExpression) expressionFactory
				.expression(SymbolicOperator.AND, boolType, xGreaterZero,
						xLessN);
		// x exists
		SymbolicExpression xExistsExpression = expressionFactory.expression(
				SymbolicOperator.EXISTS, universe.realType(), xConstant,
				zeroLessXLessN);
		...
		NumericSymbolicConstant x1Constant = (NumericSymbolicConstant) universe
				.symbolicConstant(xString, realType);
		// 0 < x'1
		xGreaterZero = (BooleanExpression) expressionFactory.expression(
				SymbolicOperator.LESS_THAN, boolType, universe.zeroReal(),
				x1Constant);
		// x'1 exists
		SymbolicExpression x1ExistsExpression = expressionFactory.expression(
		...
		// x'2 exists
		SymbolicExpression x2ExistsExpression = expressionFactory.expression(
				SymbolicOperator.EXISTS, universe.realType(), x1Constant,
				xGreaterOne);

		// // 0 < x'1 && 1 < x'2
		// BooleanExpression andExpression = (BooleanExpression)
		// expressionFactory
	...
	// TODO: this test is throwing a NullPointerException
	// because cvcProver is null. FIX ME!!!!!
	@Test
	@Ignore
	public void testTranslateExistsAnd() {
		...
		// x'1 real
		SymbolicConstant x1Real = universe.symbolicConstant(xString, realType);
		// x'1 < 10
		BooleanExpression x1LessTen = (BooleanExpression) expressionFactory
				.expression(SymbolicOperator.LESS_THAN, boolType, x1Real, ten);
		// (E x'1 . x'1 < 10)
		BooleanExpression x1Exists = universe.exists(x1Real, x1LessTen);
		// (E x . x > 0) ^ (E x'1 . x'1 < 10)
		SymbolicExpression xandx1Exist = expressionFactory.expression(
				SymbolicOperator.AND, boolType, xExists, x1Exists);
		// cvc3 translation
		List<Expr> vars = new ArrayList<Expr>();
	...
	public void testTranslateExistsNested() {
		...
		BooleanExpression xLTten = (BooleanExpression) expressionFactory
				.expression(SymbolicOperator.LESS_THAN, boolType, xInt2, ten);
		// x3 int
		...
		BooleanExpression xLTzero = (BooleanExpression) expressionFactory
				.expression(SymbolicOperator.LESS_THAN, boolType, xInt3,
						universe.zeroInt());

		// this and method returns in a random order, so doesn't help for
		// testing
		BooleanExpression xSeriesExists = universe.and(Arrays
				.asList(new BooleanExpression[] {
	...
	public void testForAll() {
		// make array
		SymbolicConstant values = universe.symbolicConstant(
				universe.stringObject("values"), intArrayType);
		// fill array
		// universe.arrayWrite(values, universe.zeroInt(), oneInt);
		// universe.arrayWrite(values, universe.oneInt(), fiveInt);
		// universe.arrayWrite(values, twoInt, twoInt);

		BooleanExpression valuesPositive = (BooleanExpression) expressionFactory
				.expression(SymbolicOperator.LESS_THAN, boolType,
						universe.zeroInt(), values);

		SymbolicExpression forAll = expressionFactory.expression(
				SymbolicOperator.FORALL, boolType, values, valuesPositive);
		Expr translateResult = cvcProver.cleanAndTranslate(forAll);

		List<Expr> vars = new ArrayList<>();
		vars.add(cvcProver.translate(values));
		Expr body = cvcProver.translate(valuesPositive);
		Expr expected = vc.forallExpr(vars, body);
