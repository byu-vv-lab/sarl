...
import edu.udel.cis.vsl.sarl.collections.common.CljSequence;
...
public class CVC3TranslateReadWriteTupleTest {
	...
	/**
	 * testTranslateTupleRead creates two ArrayLists one for numeric expressions
	 * and one for symbolic integer type. The test then translates symbolic
	 * expressions that use the symbolic operator TUPLE_READ and compares it to
	...
	public void testTranslateTupleRead() {
		...
		SymbolicExpression s2 = expressionFactory.expression(
				SymbolicOperator.TUPLE_READ, s1.type(), s1,
	...
	/**
	 * testTranslateTupleWrite creates two ArrayLists one for numeric
	 * expressions and one for symbolic integer type. The test then translates
	 * symbolic expressions that use the symbolic operator TUPLE_WRITE and
	 * compares it to the validity checker.
	 */
	...
	/**
	 * testTranslateDenseTupleWrite translates symbolic expressions that use the
	 * symbolic operator DENSE_TUPLE_WRITE and TUPLE_READ. The test uses the
	 * validity checker and compares translations using numeric integer
	 * expressions with the created translations using DENSE_TUPLE_WRITE and
	 * TUPLE_READ.
	 */
	...
	public void testTranslateDenseTupleWrite() {
		...
		SymbolicSequence<SymbolicExpression> tupleSequenceWrite = new CljSequence<SymbolicExpression>();
		...
		SymbolicExpression s3 = expressionFactory.expression(
				SymbolicOperator.DENSE_TUPLE_WRITE, s1.type(), s1,
				tupleSequenceWrite);
		SymbolicExpression s4 = expressionFactory.expression(
				SymbolicOperator.TUPLE_READ, s3.type(), s3,
				universe.intObject(0));
		SymbolicExpression s5 = expressionFactory.expression(
				SymbolicOperator.TUPLE_READ, s3.type(), s3,
				universe.intObject(1));
		SymbolicExpression s6 = expressionFactory.expression(
				SymbolicOperator.TUPLE_READ, s3.type(), s3,
