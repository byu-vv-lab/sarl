...
public class FloatModelTest {
	...
	public void test() {
		...
		if (debug) System.out.println("difference  = " + difference);
		pos = positivize(difference);
		if (debug) System.out.println("positivized = " + pos);
		abs = (NumericExpression) universe.mapSubstituter(emap).apply(pos);
		if (debug) System.out.println("upperBound1 = " + abs);
		abs = (NumericExpression) universe.mapSubstituter(xmap).apply(abs);
		if (debug) System.out.println("upperBound2 = " + abs);
		factorization = ((Polynomial) abs).factorization(idealFactory);
		if (debug) System.out.println("            = " + factorization);
	}
}
